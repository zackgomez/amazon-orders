...
class Selectors:
    ...
    # a Shipment apiece. They are therefore one selector rather than three entries: ``util.select()``
    # stops at the first entry that matches anything, so a later entry would be unreachable on exactly
    # the Orders that need it, and those Items would be dropped from the Order and its Shipment both.
